...
import { fetchSelectedEvents, modify } from "../redux/slices/searchResultsSlice";
import { format_date } from "../format/format-input";
import dayjs from "dayjs";
import { Field, Label, Input } from "@headlessui/react";
export default function MapDateInput({ name, inputLabel }) {
  ...
  const handleChange = (event) => {
    ...
    dispatch(modify({ field: "displayedEvents", value: [] }));
    dispatch(modify({ field: name, value: newValue }));
    dispatch(fetchSelectedEvents({ pagination: false }));
