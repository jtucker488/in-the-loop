...
import ExcelFileUpload from './excel-input'
...
export default function ICSInputPage({ closeModal }) {
  ...
  const handleSubmit = (event) => {
    ...
    if (selectedFile) {
      console.log("here44", selectedFile);
  ...
  return (
    <>
      <div className="flex flex-col bg-neutral-800 gap-4 p-8">
        <ExcelFileUpload setSelectedFile={setSelectedFile} /> {/* Pass the setSelectedFile callback */}
        <button
